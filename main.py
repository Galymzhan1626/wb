...
# Магазины, у которых есть вкладка ЭСФ
SHOPS_WITH_ESF = ["Абеденов"]
...
# --- ВКЛАДКИ: Поставка + ЭСФ (если магазин поддерживает) ---
st.subheader(f"🚚 Поставка: {selected_shop}")

if selected_shop in SHOPS_WITH_ESF:
    tab_supply, tab_esf = st.tabs(["📦 Расчёт поставки", "📝 ЭСФ"])
else:
    tab_supply = st.container()
    tab_esf = None

# ==========================================
# БЛОК 1: РАСЧЁТ ПОСТАВКИ
# ==========================================
with tab_supply:
...
# ==========================================
# БЛОК 2: ЭСФ (только для поддерживаемых магазинов)
# ==========================================
if tab_esf is not None:
    with tab_esf:
        ...
        if file_wb:
            try:
                # Строим словарь ТН ВЭД из прайса Google Sheets
                tnved_dict = {}
                if df_prices is not None and "Артикул" in df_prices.columns and "ТН ВЭД" in df_prices.columns:
                ...
                if not items:
                    st.warning("⚠️ Товары не найдены в файле. Проверьте формат уведомления (данные с 11-й строки, номер п/п в колонке A).")
                else:
                    st.success(f"✅ Найдено позиций: {len(items)}")
                    st.markdown("---")
                    st.subheader("📥 Выберите формат для скачивания:")

                    # ------------------------------------------
                    # ВАРИАНТ А: TXT (UTF-16 LE, для портала ЭСФ)
                    # ------------------------------------------
                    txt_lines = []
                    ...
                    for item in items:
                        qty_val = item["qty"]
                        qty_str = str(int(qty_val)) if isinstance(qty_val, float) and qty_val.is_integer() else f"{qty_val:.2f}".replace(".", ",")
                    ...
                    st.download_button(
                        label="📄 Скачать готовый ЭСФ (.txt) — Рекомендуется",
                    ...
                    # ------------------------------------------
                    # ВАРИАНТ Б: Excel-шаблон (schedule.xlsx)
                    # ------------------------------------------
                    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
                    TEMPLATE_ESF_FILE = os.path.join(BASE_DIR, "schedule.xlsx")

                    if not os.path.exists(TEMPLATE_ESF_FILE):
                        st.info("💡 Кнопка скачивания Excel (.xlsx) недоступна — файл 'schedule.xlsx' не найден в папке проекта.")
                    else:
                        try:
